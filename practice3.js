...
function isAnagram(a, b) {
  // 코드작성공간
  const formatedAString = a.replaceAll(" ", "");
  const formatedBString = b.replaceAll(" ", "");

  if (formatedAString.length !== formatedBString.length) {
    return false;
  }
  let isSameB = formatedBString;
  for (let i = 0; i < formatedAString.length; i++) {
    if (isSameB.replace(formatedAString[i], "").length !== isSameB) {
      isSameB = isSameB.replace(formatedAString[i], "");
    } else {
      return false;
    }
  }
