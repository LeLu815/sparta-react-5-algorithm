...
function findLongestPalindrome(words) {
  ...
  function isPalindrome(str) {
    ...
    const onlyTextAndNumber = toLowercaseStr.replaceAll(regex, "");
    const palindromeLength =
      onlyTextAndNumber.length % 2 === 0
        ? onlyTextAndNumber.length / 2
        : Math.floor(onlyTextAndNumber.length / 2);

    for (let i = 0; i < palindromeLength; i++) {
      const rearIndex = onlyTextAndNumber.length - 1 - i;
      if (onlyTextAndNumber[i] !== onlyTextAndNumber[rearIndex]) {
        return "";
      }
    }
    return [originalValue, onlyTextAndNumber, onlyTextAndNumber.length];
  }

  let longestLength = 0;
  let longestList = [];
  for (let i = 0; i < words.length; i++) {
    const palindrome = isPalindrome(words[i]);
    if (palindrome[2] > longestLength) {
      longestLength = palindrome[2];
      longestList = [palindrome];
    } else if (palindrome[2] === longestLength) {
      longestList.push(palindrome);
    }
  }

  if (longestList.length === 1) {
    return longestList[0][0];
  }
  if (longestList.length > 1) {
    // longestList.sort((a, b) => a[1] - b[1]);
    longestList.sort((a, b) => a[1].localeCompare(b[1]));
    return longestList[0][0];
  }
  return "";
}
